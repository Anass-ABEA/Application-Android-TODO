...
import com.thexcoders.mytodo.adapters.EmptyTodoListAdapter;
import com.thexcoders.mytodo.adapters.ListAdapter;
import com.thexcoders.mytodo.database.DbController;
import com.thexcoders.mytodo.database.SQLExecution;
import com.thexcoders.mytodo.modals.TodoModal;
import com.thexcoders.mytodo.model.Todo;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void initVariables(){
        ...
        todoListener = new ListAdapter.TodoListener() {
            ...
            public void onLongClick(Todo todo, int pos) {
                TodoModal.editModal(MainActivity.this, (int) (width * 0.8), todo, new TodoModal.ModalClickListeners() {
                    @Override
    ...
    public void clickListeners(){
        ...
        btnAddTODO.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new TodoModal(MainActivity.this, (int) (width * 0.8), new TodoModal.ModalClickListeners() {
                    @Override
                    public void saveListener(Todo todo) {
                        saveToDatabase(todo);
                    }
    ...
    public void setupAndFillList(){
        new SQLExecution(this).runWithResult(new SQLExecution.SQLWithReturn() {
            @Override
            public void execute() {
                todoList = new DbController(MainActivity.this).getAllTodos();
            }
    ...
    private void deleteFromDatabase(int pos) {
        new SQLExecution(this).runWithResult(new SQLExecution.SQLWithReturn() {
            @Override
            public void execute() {
                new DbController(MainActivity.this).deleteTodo(todoList.get(pos).getId());
            }

            @Override
            public void executeOnUI() {
                listAdapter.removeTodo(pos);
                if(todoList.size()==0) todoRecyclerView.setAdapter(new EmptyTodoListAdapter());
            }
        });
    }
